...
type Context <: Container
    ...
    form_children::List{OneOrMore{FormPrimitive}}
    property_children::List{OneOrMore{PropertyPrimitive}}
    ...
    function Context(box::BoundingBox,
                     ...
                     mirror,
                     container_children::List{Container},
                     form_children::List{Union{FormPrimitive, AbstractArray}},
                     property_children::List{Union{PropertyPrimitive, AbstractArray}},
                     order::Int,
                     clip::Bool,
                     withjs::Bool,
                     withoutjs::Bool,
...
function context(x0=0.0w,
    ...
    return Context(BoundingBox(x_measure(x0), y_measure(y0),
                   ...
                   mirror, ListNull{Container}(), ListNull{OneOrMore{FormPrimitive}}(),
                   ListNull{OneOrMore{PropertyPrimitive}}(), order, clip,
                   withjs, withoutjs, raster, minwidth, minheight, penalty)
end
...
function compose!{P <: FormPrimitive}(
        a::Context,
        b::Union{FormPrimitive, AbstractArray{FormPrimitive}, AbstractArray{P}}
    )
...
function compose!{P <: PropertyPrimitive}(
        a::Context,
        b::Union{AbstractArray{PropertyPrimitive}, AbstractArray{P}})
...
function compose!{P <: FormPrimitive}(
        a::Context,
        b::Union{AbstractArray{FormPrimitive}, AbstractArray{P}}
    )
    a.form_children = cons(b, a.form_children)
    return a
end

function compose!(a::Context, b::Union{FormPrimitive, PropertyPrimitive})
    a.form_children = cons(b, a.form_children)
    return a
end
...
function compose{P <: PropertyPrimitive}(
        a::Context,
        b::Union{PropertyPrimitive, AbstractArray{PropertyPrimitive}, AbstractArray{P}}
    )
    a = copy(a)
    compose!(a, b)
    return a
